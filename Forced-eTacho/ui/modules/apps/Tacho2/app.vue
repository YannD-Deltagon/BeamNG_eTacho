<!--
  ...
  This file OVERRIDES the stock ui/modules/apps/Tacho2/app.vue through the
  virtual filesystem: BeamNG mounts mod content over base game content, so
  while this mod is enabled the game loads this file instead of its own. The
  app.json IS overridden too, but only to set "interactive": "yes" -- without
  it AppHost keeps the widget at pointer-events: none and the settings button
  can never be clicked. The directive and DOM element are untouched, so an
  existing HUD layout keeps working and uninstalling restores the stock dial.
-->
<template>
